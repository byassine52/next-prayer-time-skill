...
const HELP_MESSAGE = 'You can say tell me what is next prayer time, or, you can say exit... What can I help you with?';
...
const GetNextPrayerTimeHandler = {
	canHandle(handlerInput) {
		...
		return (
			...
			(request.type === 'IntentRequest' && request.intent.name === 'GetNextPrayerTimeIntent')
		);
	},
	handle(handlerInput) {
...
function getNextPrayerResult(now) {
	...
	return `${nextPrayer.name} prayer at ${nextPrayer.time}. The time now is ${now}.`;
}
...
exports.handler = skillBuilder
	.addRequestHandlers(GetNextPrayerTimeHandler, HelpHandler, ExitHandler, FallbackHandler, SessionEndedRequestHandler)
